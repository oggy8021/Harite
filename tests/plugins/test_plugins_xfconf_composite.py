...
def test_composite_token_matching(monkeypatch):
    # xfconf available and lists props that include both monitor index and resolution
    monkeypatch.setattr(shutil, "which", lambda name: "/usr/bin/xfconf-query" if name == "xfconf-query" else None)

    sample_props = "/backdrop/screen0/monitor1/image_2048x1280\n/backdrop/screen0/workspace0/last-image\n"
    ...
    # applying should succeed by composite token matching (index + resolution)
