...
**後半（§6 確定 — [two-screen 整理メモ](20260611-two-screen-display-params-clarification.md#6-将来整理の方向オーナー判断確定-2026-06-12)）**

| 項目 | 方針 |
| --- | --- |
| **1:1** | 画像枚数＝ディスプレイ枚数。1ディスプレイ複数枚（tile）は非サポート。 |
| **`two_screen`** | UI/CLI/設定/embed から **全面露出撤去**。内部フラグは必要なら残す。 |
| **`l/r-display`** | 上級者 override に格下げ。**手動指定の実需要が無ければ廃止**（後半 spec で点検）。 |
| **`resolution`** | 名称是正（合成キャンバス）。低解像度出力＋auto-split 用に当面残す。**`xx%` 指定案**で `WxH` 露出縮小の可能性。 |

後半は spec 改定 → CLI/GUI/settings/embed の露出削除。v2.0.0 同梱想定。
