...
- **GUI には既に答えがある:** 検出1台では片側コントロールを無効化し、1枚運用で動く。
- **CLI での未決:** 検出1台かつ **2枚入力**したとき —
  - (a) 1枚目だけ適用、または
  - (b) **エラー**
- **現時点の傾向:** **エラーでもよい**（半分キャンバス第3経路と同様、曖昧なフォールバックを増やしたくない）。
- **ただし本稿では未確定:** **四重露出をどう解くか**（`two_screen` 廃止、`resolution` / `l/r-display` の格下げ、WorkSpace 一本化の形）が決まってから、エラー文言と override 経路を確定する。
- **two-screen 整理メモ §6** の候補は MAT-21 の設計入力とする。単体では core 変更に入らない方針は維持し、**本 roadmap 承認後に spec 改定へ**.
