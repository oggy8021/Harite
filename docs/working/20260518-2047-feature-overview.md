...
| C-05 | slideshow source 強化 | slideshow の source を単発 directory から、複数 source・source profile・将来の外部 source へ広げる。初期スコープは local directory、同期済み cloud folder、ローカル mount 済み NAS/SMB/WebDAV directory までとし、それ以上の直接連携は将来余裕がある場合に限る。 | **spec 段 1 着手** — planning #382 済、[source-spec](../specs/source/harite-source-spec.md) / [slideshow-spec](../specs/slideshow/harite-slideshow-spec.md) |
