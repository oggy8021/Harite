...
最終更新: 2026-06-08  
ステータス: **planning 着手**（§9 slice 合意 → gui-spec 改訂 → impl）
...
| # | 論点 | 提案 | オーナー |
| --- | --- | --- | --- |
| P8-1 | notebook 2 tab（Main + Slideshow） | 採用 | |
| P8-2 | 4 margin spin を Main 正面常設（compose 外周） | 採用 | |
| P8-3 | embed / notebook / position は Drawer 内 | 採用 | |
| P8-4 | トリガ label `More margin options…` | 採用（rename 可） | |
| P8-5 | Drawer 開閉 = P-07 同型スタイル | 採用 | |
| P8-6 | position selector は Main 十字と統合しない | 維持 | |

**gate 通過後:** gui-spec §3 改訂 PR → テスト → impl（GTK/Qt）。
