...
| # | 論点 | 提案 | オーナー |
| --- | --- | --- | --- |
| P8-1 | notebook を Main + Slideshow の **2 tab** | Margins tab 廃止 | |
| P8-2 | 4 margin spin を Main 正面 **常設** | compose 外周 | |
| P8-3 | embed / margin text / position は **Drawer 内** | 操作は現 Margins tab と同じ | |
| P8-4 | トリガ `More margin options…` | Slideshow と対称（rename 可） | |
| P8-5 | Drawer 開閉視認性 | P-07 同型（palette chrome tint + 上辺線 + chevron） | |
| P8-6 | position と Main direction 十字 | **統合しない**（別 widget） | |
| P8-7 | margin tooltip 3 件 | 現 gui-spec §3 Margins 表のまま載せ替え | |

## 合意後の出口

1. checklist → planning 正本 §7 gate 記入。
