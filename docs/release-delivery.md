...
- 対象リリースは **`v2.0.1`**。
- [pyproject.toml](pyproject.toml) の version は **`2.0.1`**。
...
**対象:** v2.0.0 → v2.0.1 など、同一 major の patch 更新。Breaking change は [CHANGELOG.md](CHANGELOG.md) を参照。
...
pipx install --force /abs/path/to/dist/harite-2.0.1-py3-none-any.whl
# distro PyQt6 利用時（初回と同型）:
# pipx install --system-site-packages --force /abs/path/to/dist/harite-2.0.1-py3-none-any.whl

# pip --user の場合:
python3 -m pip install --user --upgrade /abs/path/to/dist/harite-2.0.1-py3-none-any.whl

harite --version   # → 2.0.1
harite-qt
```
...
dist\windows\harite\harite.exe --version   # → 2.0.1
