...
- 解消済み: XFCE 実機でのクリーン install と GUI 起動導線確認は完了している。
- 解消済み: `v1.0.0` のタグ作成と GitHub Release 公開は完了している。
- 未完了: post-release 監視項目の確認は 2026-06-02 に実施予定である。
...
- [x] GitHub Actions の必須 CI が release 候補コミットで成功している。
...
- [x] リリースタグ作成手順を実行し、参照コミットを確認した。（`v1.0.0` / target: `main`）
- [x] リリース後の監視ポイント（不具合受付窓口、初期フィードバック確認日）を決めた。

監視ポイント（v1.0.0）

- 不具合受付窓口: GitHub Issues
- 初期フィードバック確認日: 2026-06-02
