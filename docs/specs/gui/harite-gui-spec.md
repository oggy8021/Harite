...
- **Drawer 開閉視認性（P-07）:** 開時は drawer 面板を **`QPalette.Window = AlternateBase` + `autoFillBackground`**（子 widget へ QSS を当てない）、上辺は 1px `QFrame`、トリガのみ scoped QSS + chevron up（閉=down）。`Fewer…` / `More…` ラベル反転は維持。入口: `slideshow_options_drawer.apply_slideshow_options_drawer_open_state`。
