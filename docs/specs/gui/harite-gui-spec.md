...
- **Linux IME（Qt）:** `harite-qt` 起動時に `prepare_qt_input_method_env()` が `GTK_IM_MODULE` / `XMODIFIERS` から `QT_IM_MODULE` を補完する。**fcitx + pip PyQt6:** distro の fcitx Qt6 プラグイン（`fcitx5-frontend-qt6`）は **pip 同梱 Qt6 と非互換**（`Qt_6_PRIVATE_API` 未定義シンボルでロード失敗；MAT-06 viper3 確定）。Harite は pip venv へ symlink **しない**（既存 symlink は起動時に削除）。**回避:** distro `python3-pyqt6`（Debian/Ubuntu/Mint: apt、`--system-site-packages` venv）— その場合のみシステム fcitx プラグインを symlink 補助可。**distro PyQt6 前提:** OS に `fcitx5-frontend-qt6` が入っていること。`keyword(CODH)` は `configure_text_input_widget` で IME 有効化。Windows は本節の対象外。
