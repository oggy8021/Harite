...
- Watch 全体は controller が無い外周 gutter を残し、source block が tab の左右端へ寄り切らないようにする。
- 上下方向のバランスも末端 widget ではなく親コンテナで取り、中段主面が残る見え方を優先する。
...
- `Left/Right` の control 群が上寄せへ崩れる場合は、各アーム内部を 3 行化し、中央行へ置く方を優先する。
