...
最終更新: 2026-05-15
対象: Phase10 Main Window / Watch icon mock

## 目的

- Phase10 4th planning で決めた Main Window / Watch icon 導入候補を、実装前に低コストで見比べる。
- XFCE 実機確認の前段として、Main Window と Watch 面の tone、密度、意味差の見え方を早めに掴む。
...
- 本メモは [docs/specs/gui/gui-phase10-4th-planning.md](docs/specs/gui/gui-phase10-4th-planning.md) の「Main Window icon 適用前の軽量 HTML mock」を具体化するための薄いテンプレである。
- 実ファイルは [docs/specs/gui/gui-phase10-icon-mock.html](docs/specs/gui/gui-phase10-icon-mock.html) とし、Main Window 面と Watch 面を 2 セクションで持つ。
- 本メモで見るのは実装完成度ではなく、Main Window / Watch 面の当たり、icon set 全体の tone、文字併記の要否である。
- ただし Watch 面は Main Window より情報量が少なく、縦横比も異なるため、Main Window と同列の重み比較には使わない。
...
- HTML mock は 1 枚とし、Main Window mock と Watch mock を縦積みで置き、横幅は揃える。
...
- mock では段階適用の順番は持ち込まず、今回見たい Main Window / Watch の icon 候補を最初から全部並べる。
...
- Watch 面は、Main Window と重みを見比べるためではなく、pair readability と左右戦略の見え方だけを見る補助面とする。
...
- 文字併記: なし / 短縮文字列あり
- feature 初手: 短縮文字列あり
- 対象面: Main Window のみ / Main Window + Watch
- window 幅メモ: [value]
- icon size: [value]
- stroke width: [value]
- gap / padding: [value]
...
- Main Window / Watch icon 面の当たりを HTML mock で一度確認している。
